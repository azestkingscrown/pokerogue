...
export class SceneBase extends Phaser.Scene {
  ...
  public loadImage(key: string, folder: string, filename = `${key}.png`): this {
    ...
    if (folder.startsWith("ui")) {
      const legacyFolder = folder.replace("ui", "ui/legacy");
      this.load.image(`${key}_legacy`, getCachedUrl(`images/${legacyFolder}/${filename}`));
    }
    return this;
  }
  ...
  public loadSpritesheet(key: string, folder: string, size: number, filename = `${key}.png`): this {
    ...
    if (folder.startsWith("ui")) {
      const legacyFolder = folder.replace("ui", "ui/legacy");
      this.load.spritesheet(`${key}_legacy`, getCachedUrl(`images/${legacyFolder}/${filename}`), {
        frameWidth: size,
        frameHeight: size,
      });
    }
